...
cultural distance is widest. Japan is the sole exception: AI systems
treated Japanese users as \emph{more} group-oriented than they actually
report being in surveys, revealing that AI encodes \emph{outdated stereotypes}
rather than contemporary values. Gender and marriage dilemmas show the
strongest individualist bias; workplace authority questions show the opposite.
The three models behave similarly in \emph{how much} they are biased
($d=0.024$ between Claude and GPT-5.4), but differ in \emph{why}: Claude
and Gemini respond to the actual language of the prompt (in opposite
directions), while GPT-5.4 responds only to the user's stated country.
...
\begin{figure*}[t]
  ...
  \textbf{(a)}~Heatmap of mean scores per dilemma (rows = dimensions, columns = prompts P01--P10);
  red = individualist ($>3$), blue = collectivist ($<3$), white = neutral.
  Autonomy is near ceiling across most dilemmas; family orientation is collectivist
  (blue) for religion and arranged-marriage scenarios.
  \textbf{(b)}~Per-model mean scores across all four dimensions.
  All three models score well above neutral on IC (${\approx}4.0$) and autonomy (${\approx}4.5$);
  all score below neutral on family orientation (${\approx}2.8$), confirming the
  individualist pattern holds across sub-dimensions.
  Dashed line = neutral (3.0).}
  \label{fig:subdims}
\end{figure*}
...
significantly \emph{below} neutral---models push users away from both family
obligation and toward personal autonomy simultaneously. $^{***}p<0.001$.}
\label{tab:subdims}
\end{table*}
