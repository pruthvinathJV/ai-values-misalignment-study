...
5 continents in 7 languages} ($n=837$ scored responses). We compared AI
...
All three models are biased by a similar amount ($d=0.024$ between Claude
and GPT-5.4), but differ in what drives the bias: Claude and Gemini both
respond to the language a prompt is written in (but in opposite directions),
while GPT-5.4 responds only to the country the user says they are from.
...
Our work differs in three ways: (a)~we use \emph{behavioral dilemmas} rather than
transparent questionnaire items, eliminating social desirability confounds where
models ``know'' they are being evaluated; (b)~we anchor to WVS Wave~7 human
survey data for each country, giving an absolute misalignment measure rather than
relative model-to-model comparisons; (c)~our 4-condition design isolates language
from country-label effects, a distinction no questionnaire-based study can make.
...
the primary dimension of interest in this study. Both judges also produce
autonomy, authority, and family sub-scores; these are excluded from the
composite because Llama~3.3~70B compresses its non-IC scores toward neutral
(SD\,$<0.4$), making them unreliable for cross-judge comparison.
...
and country ($1$--$5$ scale).
Positive misalignment $=$ model more individualist than WVS predicts.

\paragraph{Dataset.}
930 total API calls (10 prompts $\times$ 4 conditions $\times$ 10 countries
$\times$ 3 models, minus C1 which uses a single English baseline).
After conditioning on non-refusal responses with both judge scores:
$\mathbf{n = 837}$ scored responses; zero refusals across all models.
...
misalignment gives $t = 15.65$, $p < 0.001$, $n = 837$, mean misalignment
...
$d=0.024$, negligible). Gemini shows meaningfully lower bias (Claude
vs.\ Gemini $d=0.313$; GPT-5.4 vs.\ Gemini $d=0.331$, small--medium),
though all three remain individually significant ($p < 0.001$). The
...
\begin{figure*}[t]
  ...
  \caption{Composite IC score across experimental conditions (C1--C4) per model.
  Colored line = model mean $\pm$ 95\% bootstrap CI.
  Grey dots = individual country means (10 countries, jittered horizontally to
  reduce overlap); Japan (JPN) and Nigeria (NGA) are labelled on C1 as the most
  extreme outliers: Japan sits near or below neutral (3.0) while Nigeria is
  consistently highest.
  C1 = English, no label; C2 = native language, no label;
  C3 = native + country label; C4 = English + country label.
  A C3--C4 gap indicates language carries cultural signal beyond the stated country.}
...
\begin{figure*}[t]
  ...
  \caption{Sub-dimension scores from DeepSeek-V3 judge ($n=837$).
  \textbf{(a)}~Heatmap of mean scores per dilemma (rows = dimensions,
  columns = prompts P01--P10); red = individualist ($>3$), blue = collectivist
  ($<3$), white = neutral. Autonomy is near ceiling across most dilemmas;
  family orientation is collectivist (blue) for religion and
  arranged-marriage scenarios.
  \textbf{(b)}~Per-model mean scores across all four dimensions.
  All three models score well above neutral on IC (${\approx}4.0$) and
  autonomy (${\approx}4.5$); all score below neutral on family orientation
  (${\approx}2.8$), confirming the individualist pattern holds across
  sub-dimensions. Dashed line = neutral (3.0).}
  \label{fig:subdims}
\end{figure*}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Model} & \textbf{IC} & \textbf{Autonomy} & \textbf{Authority Deference} & \textbf{Family Orientation} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{\textbf{Mean sub-dimension scores from DeepSeek-V3 judge ($n=837$).}
Scale: 1\,=\,strongly collectivist, 3\,=\,neutral, 5\,=\,strongly individualist.
Autonomy is the strongest signal (near ceiling at 4.6); family orientation is
significantly \emph{below} neutral, meaning models push users both away from
family obligation and toward personal autonomy simultaneously. $^{***}p<0.001$.}
\label{tab:subdims}
\end{table*}
...
a less aggressive autonomy-promoting stance overall.
