...
**Pruthvinath Jeripity Venkata** — Independent Researcher

---

## Overview

This repository contains the full replication package for the paper. We audited three frontier LLMs across **10 countries, 7 languages, and 10 culturally-sensitive personal dilemmas** (n=837 scored responses), measuring individualism–collectivism bias relative to World Values Survey Wave 7 ground truth.
...
```
reproduce/
├── experiment.db              # Full SQLite database (all runs, responses, scores)
├── requirements.txt           # Python dependencies
├── .env.example               # API key template
├── data/
│   └── wvs_anchor_scores.csv  # WVS Wave 7 anchor scores per country/prompt
├── notebooks/
│   ├── 01_setup_db.py         # Create DB schema, populate prompts & run table
│   ├── 02_run_experiments.py  # Call 3 LLMs — saves responses to DB
│   ├── 03_score_responses.py  # LLM judging (Llama + DeepSeek) + lexical baseline
...
│   └── 06_extra_figures.py    # Figure 0 (pipeline) + Figure 5 (sub-dimensions)
└── paper/
...
The `experiment.db` already contains all 930 responses and scores.  
...
Requires ~$15–20 in API costs and several hours.

```bash
python3 notebooks/01_setup_db.py       # Set up fresh DB
python3 notebooks/02_run_experiments.py # ~930 API calls (Claude + GPT-5.4 + Gemini)
...
Each notebook is **resumable** — safe to interrupt and restart at any time.
...
- **Secondary:** DeepSeek-V3 sub-dimension scores (autonomy, authority, family) — see Section 4.5
